...
public class TutorialSystem : MonoBehaviour
{
    ...
    GameObject tutText;
    string tutorialText = "Welcome to pacman reborn!";
    string currentText = "";
    int showingChars = 0;
    float lastCharAdded = 0f;

    // Start is called before the first frame update
    void Start()
    {

        //setup tutorial objects
        border = GameObject.Find("Border");
        hearts = GameObject.FindGameObjectWithTag("Lives");
        scoreBoard = GameObject.FindGameObjectWithTag("Score");
        tutText = GameObject.FindGameObjectWithTag("Tutorial");
        ...
        //spawn player in the middle of the box for the tutorial
        player = Instantiate((GameObject)Resources.Load("Player"), Vector3.zero, Quaternion.identity);
    ...
    void NextTutorialPage() {
        ...
        showingChars = 0;
        if (tutorialPage == 1) {
            tutorialText = "Use the arrow or WASD keys to move!";
        }

        if (tutorialPage == 2)
        {
            Destroy(border);
            tutorialText = "Don't get hit by the enemies!";
        }

        if (tutorialPage == 3)
        {
            scoreBoard.SetActive(true);
            tutorialText = "Eat the food to increase your score and scare away enemies!";
        }

        if (tutorialPage == 4)
        {
            hearts.SetActive(true);
            tutorialText = "Remember that you only have 3 lives. Good luck and enjoy playing!";
    ...
    void Update()
    {
        if (Time.time - lastCharAdded > .05f) {
            lastCharAdded = Time.time;
            showingChars = Mathf.Min(tutorialText.Length, showingChars+1);
        }
        currentText = tutorialText.Substring(0, showingChars);
        tutText.GetComponent<TextMeshProUGUI>().text = currentText;
        GameObject.FindGameObjectWithTag("clickThing").GetComponent<RectTransform>().localPosition = new Vector3(-92.92f, 250.6f + Mathf.Sin(Time.time * 10f), 0);

        if (Input.GetMouseButtonDown(0)) {
            if (showingChars == tutorialText.Length) {
                NextTutorialPage();
            }
        }
